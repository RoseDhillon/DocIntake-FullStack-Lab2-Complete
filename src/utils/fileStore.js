...
async function ensureDirs() {
  ...
  if (STORAGE_MODE === "sync") {
    ...
    if (!fs.existsSync(PATHS.DOC_INDEX))
      fs.writeFileSync(PATHS.DOC_INDEX, "[]", "utf8")
    if (!fs.existsSync(PATHS.AUDIT_LOG))
      fs.writeFileSync(PATHS.AUDIT_LOG, "", "utf8")
    return
  }
  for (const d of dirs) await fsp.mkdir(d, { recursive: true })
  try {
    await fsp.access(PATHS.DOC_INDEX)
  } catch {
    await fsp.writeFile(PATHS.DOC_INDEX, "[]", "utf8")
  }
  try {
    await fsp.access(PATHS.AUDIT_LOG)
  } catch {
    await fsp.writeFile(PATHS.AUDIT_LOG, "", "utf8")
  }
}
